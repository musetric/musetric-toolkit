...
_NNAUDIO_NOISE_PATTERNS = [
    re.compile(r"^Creating .* filter"),
    re.compile(r"^Low pass filter created"),
    re.compile(r"^Early downsampling filter created"),
    re.compile(r"^No early downsampling is required"),
    re.compile(r"^num_octave\s*="),
]
...
def build_stream_suppression_patterns(
    ...
    if log_level == "debug":
        return []
    return list(_NNAUDIO_NOISE_PATTERNS)
...
def configure_third_party_logging(log_level: str) -> None:
    ...
    for logger_name in ("torch", "torchaudio", "skey"):
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Detect musical key (root + mode) with S-KEY",
    )
    parser.add_argument(
        "--audio-path",
        required=True,
    ...
    parser.add_argument(
        "--result-path",
        required=True,
        help="Path to write key JSON result",
    )
    parser.add_argument(
        "--models-path",
...
def main_cli() -> None:
    ...
    try:
        from musetric_toolkit.key_audio.main import main  # noqa: PLC0415

        main(args)
    except Exception:
        logging.exception("Key detection failed")
        sys.exit(1)
